"""
Example: Generate motion with densified (new) agents.

This example demonstrates how to:
1. Load a pretrained SceneStreamer model from HuggingFace
2. Load a scenario dataset
3. Generate motion trajectories with additional densified agents

The difference from motion-only generation is that this approach
can add new agents to the scenario, increasing traffic density.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate SceneStreamer densified predictions from a HuggingFace checkpoint."
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    for count, data_dict in enumerate(tqdm.tqdm(iterable, total=total)):
        ...
        densified_data_dict, _ = generate_initial_state(data_dict=batched_data_dict, model=pl_model.model)
        densified_motion_input = convert_initial_states_as_motion_data(densified_data_dict)
        ...
        print(f"[SceneStreamer] Generated densified motion for scenario_id={scenario_id!r}.")
        out_dir = Path("artifacts/example_densified_agents/latest")
        ...
        current_agent_shape = densified_motion_input["decoder/current_agent_shape"][0].detach().cpu().numpy()
