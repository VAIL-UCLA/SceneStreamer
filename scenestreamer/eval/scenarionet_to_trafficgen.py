...
This code tries to convert the scenario description from MetaDrive to the internal data representation of TrafficGen.
And it's a complete disaster and I don't want to investigate it as what I've done in 2023. Just try to make it work with
SceneStreamer code and get the TrafficGen's evaluation metrics (aka MMD).
...
def _extract_map(map_feat, sample_num):
    ...
    for map_feat_id, map_feat in map_feat.items():
        ...
        if "polyline" not in map_feat:
            if "position" not in map_feat:
                # PZH: I just don't XXXXing care about this. (SPEED_BUMP)
                continue
...
def metadrive_scenario_to_init_data(scenario):
    ...
    PZH_TRACK_NAMES = [None] * len(tracks)
    ...
    for indx, (id, track) in enumerate(tracks.items()):
        ...
        # TODO: Width length or length width I don't XXXXing care.
        all_agent[:, indx, 5] = track[SD.STATE]['width']
        all_agent[:, indx, 6] = track[SD.STATE]['length']

        all_agent[:, indx, 7] = 1
        all_agent[:, indx, 8] = track[SD.STATE]['valid'].reshape(track_len)

        PZH_TRACK_NAMES[indx] = id

    assert sdc_index is not None

    # Make ego agent to the first place
    all_agent[:, [sdc_index, 0]] = all_agent[:, [0, sdc_index]]
    PZH_TRACK_NAMES = np.array(PZH_TRACK_NAMES)
    PZH_TRACK_NAMES[[sdc_index, 0]] = PZH_TRACK_NAMES[[0, sdc_index]]
    ...
    for step in range(track_len):
        ...
        for traffic_light_index, traffic_light in traffic_lights.items():
            ...
            if "lane" in traffic_light_state:
...
#     cnt = 0
#
#     from trafficgen_reference.traffic_generator.traffic_generator import TrafficGen
#     from trafficgen_reference.traffic_generator.utils.utils import get_parsed_args
#     from trafficgen_reference.utils.config import load_config_init
#     from trafficgen_reference.traffic_generator.utils.data_utils import process_data_to_internal_format
#
