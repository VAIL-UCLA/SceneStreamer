...
from scenestreamer.gradio_ui.plot import plot_pred, create_animation_from_pred
...
def debug_run_model(config):
    import os
    global SCENE_IDX
    path = "/bigdata/zhenghao/scenestreamer/lightning_logs/scenestreamer/1104_MidGPT_NoAgnt_WTLSgl_WContRel_WBackward_FixedStepAgentID_2024-11-04_2208/checkpoints/last.ckpt"
    omegaconf.OmegaConf.set_struct(config, False)
    config.PREPROCESSING.keep_all_data = True
    config.pretrain = "/bigdata/zhenghao/scenestreamer/lightning_logs/scenestreamer/1104_MidGPT_NoAgnt_WTLSgl_WContRel_WBackward_FixedStepAgentID_2024-11-04_2208/checkpoints/last.ckpt"
...
def evaluate_scgen(config):
    from scenestreamer.utils.safety_critical_generation_utils import _overwrite_data_given_agents_not_ooi, get_ego_edge_points, get_ego_edge_points_old, post_process_adv_traj, _overwrite_data_given_agents_ooi, _overwrite_data_given_agents, set_adv, run_backward_prediction_with_teacher_forcing
    from scenestreamer.utils import utils
    import copy
    path = "/bigdata/zhenghao/scenestreamer/lightning_logs/scenestreamer/1104_MidGPT_NoAgnt_WTLSgl_WContRel_WBackward_FixedStepAgentID_2024-11-04_2208/checkpoints/last.ckpt"
    omegaconf.OmegaConf.set_struct(config, False)
...
def debug_eval_CAT(config):
    import os
    from scenestreamer.dataset.scenarionet_utils import overwrite_gt_to_pred_field
    omegaconf.OmegaConf.set_struct(config, False)
    config.PREPROCESSING.keep_all_data = True
    config.pretrain = "/bigdata/zhenghao/scenestreamer/lightning_logs/scenestreamer/1104_MidGPT_NoAgnt_WTLSgl_WContRel_WBackward_FixedStepAgentID_2024-11-04_2208/checkpoints/last.ckpt"
    config.BACKWARD_PREDICTION = True  # <<<
