...
import scenestreamer.utils as utils
...
from scenestreamer.utils import REPO_ROOT, get_time_str
...
def main(config):
    ...
    if config.wandb and not config.eval:
        ...
        logger = WandbLogger(
            ...
            save_dir=save_dir,
            id=name,
            project="scenestreamer",
            log_model=False,
            group=exp_name,
        )
    else:
        logger = TensorBoardLogger(save_dir=save_dir / "scenestreamer", name=name)

    ckpt_save_dir = pathlib.Path(save_dir).absolute() / "scenestreamer" / name
    ...
    trainer_kwargs = dict(
        ...
        # strategy_settings={"timeout": datetime.timedelta(seconds=7200)}
    )

    # If multi GPUs are found:
    if torch.cuda.device_count() > 1:
        from lightning.pytorch.strategies.ddp import DDPStrategy
        trainer_kwargs["strategy"] = DDPStrategy(timeout=datetime.timedelta(seconds=7200))
    ...
    utils.rank_zero_print(summarize(model, max_depth=3))
    utils.rank_zero_print("==============================")
    utils.rank_zero_print("Root Directory: ", save_dir / "scenestreamer")
