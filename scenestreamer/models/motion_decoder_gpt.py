...
def get_edge_info_new(*, q_k_valid_mask, q_k_relation, relation_model, relation_model_v, require_relation_pairwise=None):
    ...
    if relation_model is not None:
        if require_relation_pairwise is not None:
            ...
            edge_features = utils.unwrap(relation_model(edge_features[require_relation]), require_relation)
        else:
...
class MotionDecoderGPT(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = MultiCrossAttTransformerDecoder(
            decoder_layer=MultiCrossAttTransformerDecoderLayer(
                ...
                dropout=dropout,
                use_adaln=self.use_destination,
        ...
        if self.use_destination:
            self.prediction_prenorm = common_layers.AdaLayerNorm(d_model, conditioning_dim=d_model, batch_first=True)

            self.map_id_embed = common_layers.Tokenizer(
                num_actions=self.config.PREPROCESSING.MAX_MAP_FEATURES, d_model=d_model, add_one_more_action=True
            )
    ...
    def randomize_modeled_agent_id(self, data_dict, clip_agent_id=False):
        ...
        return new_modeled_agent_id.long()
    ...
    def forward(self, input_dict, use_cache=False, a2a_knn=None, a2t_knn=None, a2s_knn=None):
        ...
        if self.config.BACKWARD_PREDICTION:
            ...
            special_tok_emb = special_tok_emb + in_backward_prediction_embed
        ...
        #         pass
        #     else:
        #         action_token = action_token + condition_token

        condition_token = None
        if self.use_destination:
            condition_token = utils.unwrap(
                self.map_id_embed(input_dict["decoder/dest_map_index"][action_valid_mask]),
                action_valid_mask
            )
            ...
            # We don't add map feat ID pe in SceneEncoder, so we add it here. (same code in TG dec too)
        ...
        if self.use_destination:
            ...
            output_tokens = self.prediction_prenorm(decoded_tokens[action_valid_mask], z=condition_token[action_valid_mask])
