...
class Tokenizer(nn.Module):
    ...
    def forward(self, actions):
        ...
        if new_actions.numel() > 0:
            assert new_actions.max() < self.tokens.num_embeddings
            assert new_actions.min() >= 0
...
class AdaLayerNorm(nn.Module):
    def __init__(self, hidden_size, conditioning_dim, batch_first, eps=1e-5):
        super().__init__()
        self.eps = eps
