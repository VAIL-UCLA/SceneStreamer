...
class MultiCrossAttTransformerDecoderLayer(Module):
    ...
    def __init__(
        ...
        if use_adaln:
            ...
            self.a2s_norm = common_layers.AdaLayerNorm(d_model, conditioning_dim=d_model, batch_first=True)
            self.a2t_norm = common_layers.AdaLayerNorm(d_model, conditioning_dim=d_model, batch_first=True)
            self.a2a_norm = common_layers.AdaLayerNorm(d_model, conditioning_dim=d_model, batch_first=True)
            self.mlp_prenorm = common_layers.AdaLayerNorm(d_model, conditioning_dim=d_model, batch_first=True)
        else:
    ...
    def forward(
        ...
        *,
        ...
        out = x
        if self.use_adaln:
            out = self.a2t_norm(out, z=condition_token.swapaxes(1, 2).flatten(0, 1))
        ...
        out = x
        if self.use_adaln:
            out = self.a2a_norm(out, z=condition_token.reshape(B * T, N, D))
        ...
        out = x
        if self.use_adaln:
            out = self.a2s_norm(out, z=condition_token.reshape(B, T*N, D))
        ...
        out = x
        if self.use_adaln:
            out = self.mlp_prenorm(out, z=condition_token)
        else:
