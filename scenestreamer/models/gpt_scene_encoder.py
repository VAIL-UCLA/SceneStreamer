...
from scenestreamer.models.layers import polyline_encoder, common_layers, fourier_embedding
from scenestreamer.models.layers.gpt_encoder_layer import SelfAttTransformerEncoder, SelfAttTransformerEncoderLayer
from scenestreamer.models.motion_decoder_gpt import get_edge_info, get_edge_info_new
from scenestreamer.models.ops.collapse_time import collapse_time
from scenestreamer.models.scene_encoder import find_last_valid, mode_agent_id
...
class SceneEncoderGPT(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if L != 0:
            if self.config.PREPROCESSING.REMOVE_TRAFFIC_LIGHT_STATE:
                if traffic_light_feature.ndim == 4:
                    traffic_light_feature = traffic_light_feature[:, 0]
                if traffic_light_valid_mask.ndim == 3:
                    traffic_light_valid_mask = traffic_light_valid_mask[:, :self.history_steps].any(dim=1)
        ...
        scene_info = get_edge_info_new(
            q_k_valid_mask=rel_mask,
            q_k_relation=rel_feat,
            relation_model=self.relation_embed,
            relation_model_v=self.relation_embed_v if self.config.MODEL.ADD_RELATION_TO_V else None,
        )
