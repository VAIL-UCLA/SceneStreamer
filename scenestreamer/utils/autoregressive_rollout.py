...
from scenestreamer.models.motionlm import nucleus_sampling
from scenestreamer.tokenization.gen_tokenizers import Tokens, SceneStreamerTokenizer
...
class StateMachine:
    ...
    def process_UPDATE_START(self, model_output):
        ...
        new_tokens = Tokens.concatenate(
            [action_tokens, SceneStreamerTokenizer.get_update_end_tokens(use_numpy=False, device=out.device)]
        )
        self.tokens = Tokens.concatenate([self.tokens, new_tokens])

        self.state = SceneStreamerTokenizer.UPDATE_END
    ...
    def process_UPDATE_END(self, model_output):
        ...
        out = mask_out_invalid_actions(out, valids=[SceneStreamerTokenizer.STEP_END, SceneStreamerTokenizer.REMOVE_START])
        ...
        # TODO ===== Fix in future
        if a == SceneStreamerTokenizer.REMOVE_START:
            a = SceneStreamerTokenizer.STEP_END
        # TODO ===== Fix in future

        if a == SceneStreamerTokenizer.REMOVE_START:
            # TODO double check
            self.state = SceneStreamerTokenizer.REMOVE_START
        ...
        elif a == SceneStreamerTokenizer.STEP_END:
            out = torch.cat([action, torch.as_tensor([SceneStreamerTokenizer.STEP_START], dtype=out.dtype, device=out.device)])

            self.state = SceneStreamerTokenizer.STEP_START
    ...
    def process_REMOVE_START(self, model_output):
        ...
        out = mask_out_invalid_actions(out, valids=[SceneStreamerTokenizer.REMOVE_END] + list(self.agent_ids))

        action = sample_action(out, self.config)

        if action == SceneStreamerTokenizer.REMOVE_END:
            # TODO
    ...
    def process_STEP_START(self, model_output):
        ...
        out = mask_out_invalid_actions(out, valids=[SceneStreamerTokenizer.UPDATE_START, SceneStreamerTokenizer.ADD_START])

        action = sample_action(out, self.config)

        a = action.item()
        if a == SceneStreamerTokenizer.UPDATE_START:
            # Add all existing agent_id to the tokens.

            self.state = SceneStreamerTokenizer.UPDATE_START
            ...
            out = torch.cat([action, torch.as_tensor([SceneStreamerTokenizer.STEP_START], out.dtype, out.device)])
...
class ARRollout:
    ...
    def __init__(self, init_tokens, init_valid_mask, causal_mask_offset, map_ids, agent_ids, config):
        ...
        self.states = [
            StateMachine(
                state=SceneStreamerTokenizer.UPDATE_START,
