...
def overwrite_to_scenario_description_new_agent(output_dict_mode, original_SD, ooi=None, type_convert_map=None,
    ...
    new_SD['metadata']['dataset'] = 'SceneStreamer'
...
class SceneStreamerRLScenarioGenerator:
    def __init__(self, model_name, huggingface_repo=None, huggingface_file=None, checkpoint_path=None):
        ...
        assert model_name in [
            ...
            "scenestreamer-full-large-nors"], "Model name not supported. Please use scenestreamer-full-large or scenestreamer-base-large."
        if "scenestreamer-full" in model_name:
        ...
        self.scenestreamer_generator = None
    ...
    def GPT_AR(self, input_dict):
        if self.scenestreamer_generator is None:
        ...
        with torch.no_grad():
            self.scenestreamer_generator.reset(new_data_dict=input_dict)

            if self.model_name in ["scenestreamer-full-large", "scenestreamer-full-large-nors", "scenestreamer-full-xl"]:
                output_dict = self.scenestreamer_generator.generate_scenestreamer_initial_state_and_motion(
                    progress_bar=False,
                    teacher_forcing_sdc=True,
                )
            elif self.model_name == "scenestreamer-base-large":
                output_dict = self.scenestreamer_generator.generate_scenestreamer_motion(
                    progress_bar=False,
                    teacher_forcing_sdc=True,
                )
            else:
                raise ValueError("Model name not supported. Please use scenestreamer-full-large or scenestreamer-base-large.")
    ...
    def before_episode(self, env=None, scenario_data=None):
        ...
        if sid not in self.storage:
            # from scenario_net data to scenestreamer data_dict
...
if __name__ == '__main__':
    g = SceneStreamerRLScenarioGenerator(model_name="scenestreamer-base-large")
    from scenestreamer.utils import REPO_ROOT
    import pathlib
    import pickle
    ...
    with open("sd_1a7143a44e480ca6_scenestreamer_test.pkl", "wb") as f:
