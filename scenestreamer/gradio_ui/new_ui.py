...
from scenestreamer.gradio_ui.plot import plot_gt, plot_pred, create_animation_from_pred, create_animation_from_gt
...
config_dir = Path("/absolute/path/to/conf").resolve()
with initialize_config_dir(config_dir=str(REPO_ROOT / "cfgs")):
...
default_config.TOKENIZATION.TOKENIZATION_METHOD = "bicycle_interpolated"
...
def on_data_file_select(file_path):
    ...
    scenario_data_dict = preprocess_scenario_description_for_motionlm(
        ...
        keep_all_data=True,
        # cache=None,
        tokenizer=get_tokenizer(config=state.config)
    )
...
def on_generate_button_click(
    ...
    if only_draw_gt:
        ...
        if args.display_video:
            video_path = str(REPO_ROOT / "gradio_tmp" / "gt_animation_{}.mp4".format(uuid.uuid4()))
            video_path = create_animation_from_gt(output_dict, save_path=video_path, dpi=dpi, fps=fps)
        ...
        if not only_draw_detokenized:
            ...
            output_dict = get_tokenizer(config).detokenize(
                output_dict, detokenizing_gt=False, backward_prediction=draw_backward_prediction
            )
        else:
            # ===== DEBUG =====
            ...
            output_dict = get_tokenizer(config).detokenize(output_dict, detokenizing_gt=True)
        ...
        if args.display_video:
            ...
            video_path = create_animation_from_pred(output_dict, save_path=video_path, dpi=dpi, fps=fps)
