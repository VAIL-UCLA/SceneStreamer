...
from scenestreamer.utils.safety_critical_generation_utils import (
    _overwrite_data_given_agents_not_ooi,
    get_ego_edge_points,
    get_ego_edge_points_old,
    post_process_adv_traj,
    _overwrite_data_given_agents_ooi,
    _overwrite_data_given_agents,
    set_adv,
)
...
def draw_trajectory(
    *,
    ...
    assert isinstance(width, float) or heading.shape == width.shape
    assert isinstance(length, float) or heading.shape == length.shape
    contour = cal_polygon_contour(x=pos[..., 0], y=pos[..., 1], theta=heading, width=width, length=length)
...
def draw_crosswalk(ax, polygon, mask, alpha=0.1):
    ...
    polygon = Polygon(polygon, closed=True, facecolor='gray', alpha=alpha, linewidth=0.5)
...
def _plot_map(data_dict, ax, dont_draw_lane=False):
    if "vis/map_feature" in data_dict:
        ...
        map_pos = data_dict["raw/map_feature"][:, :, :2]  # (num map, num vec, 2)
        map_feat = data_dict["raw/map_feature"]
    ...
    for map_feat_ind in range(map_pos.shape[0]):
        ...
        # if 'decoder/dest_map_index' in data_dict:
        #     # decoder/dest_map_index is temporal. So only take the dest at t=0.
        #     for i in [0, 30, 60, 90]:
        #         if map_feat_ind in data_dict['decoder/dest_map_index'][i]:
        #             use_special_color = True

        if is_crosswalk[map_feat_ind]:
            ...
            if use_special_color:
                draw_2d(map_pos[map_feat_ind], map_mask[map_feat_ind], c=(1, 0, 0, 0.2), linewidth=0.5)
            else:
                draw_2d(map_pos[map_feat_ind], map_mask[map_feat_ind], c=(0.5, 0.5, 0.5, 0.2), linewidth=0.5)
        else:
            if use_special_color:
                draw_2d(map_pos[map_feat_ind], map_mask[map_feat_ind], c=(1, 0, 0, 0.5), label="map", linewidth=1.0)
            else:
                draw_2d(map_pos[map_feat_ind], map_mask[map_feat_ind], c=(0.5, 0.5, 0.5, 0.5), label="map", linewidth=1.0)


def _plot_traffic_light(data_dict, ax, step=None):
    if "encoder/traffic_light_feature" not in data_dict:
        if "model/traffic_light_state" in data_dict:
            tl_state = data_dict["model/traffic_light_state"]
            ...
            for tl_ind in range(tl_state.shape[1]):
                if not tl_mask[step, tl_ind]:
                    continue
                tl_pos_t = tl_pos[tl_ind]
                tl_state_t = tl_state[step, tl_ind].item()
                if tl_state_t == 1:
                    color = 'green'
                elif tl_state_t == 2:
                    color = 'yellow'
            ...
            return patches
        elif "encoder/traffic_light_state" in data_dict:
            tl_state = data_dict["encoder/traffic_light_state"]
            tl_pos = data_dict["encoder/traffic_light_position"][:, :2]  # NT, 3
            tl_mask = data_dict["encoder/traffic_light_valid_mask"]  # T, NT
            patches = []
            if step is None:
            ...
            raise ValueError("No traffic light data found in the data_dict")
...
def _plot_gt(data_dict, ax, draw_line=False, draw_text=True):
    ...
    return get_limit(agent_pos=agent_pos[agent_mask], map_pos=map_pos)


def plot_gt(data_dict, get_info=False, save_path=None):
    fig = plt.figure(figsize=(10, 10), dpi=300)
    ax = fig.add_subplot(111)
    ax.set_aspect(1)

    ret = _plot_gt(data_dict, ax)
    ...
    if hasattr(canvas, "tostring_rgb"):
        ret = PIL.Image.frombytes("RGB", (w, h), canvas.tostring_rgb())
    else:
        # Matplotlib>=3.10 removed tostring_rgb on Agg; use the RGBA buffer.
        rgba = np.asarray(canvas.buffer_rgba())
        ret = PIL.Image.fromarray(rgba[..., :3], mode="RGB")
    ...
    if get_info:
        # TODO: What is info dict???
        return ret, {}
    return ret


def plot_pred(data_dict, show=False, save_path=None):
    ...
    if "decoder/object_of_interest_id" in data_dict:
        modeled_agents_indicies = np.concatenate(
            [data_dict["decoder/object_of_interest_id"],
             np.atleast_1d(ego_agent_id)]
        )
    else:
    ...
    with writer.saving(fig, save_path, dpi=dpi):
        for t in range(T):
            ...
            for agent_ind, (rect, text) in enumerate(zip(agent_patches, agent_texts)):
                ...
                raise ValueError("wrong animation")
...
def _animate_trafficgen(
    ...
    # T = agent_pos.shape[0]  # Number of timesteps
    N = agent_pos.shape[0]  # Number of agents
    ...
    with writer.saving(fig, save_path, dpi=dpi):
        ...
        for agent_ind in range(N):
            if not draw_traffic and agent_ind not in labeled_agent_id:
                continue
            face_color = cmap[0] if agent_ind == ego_agent_id else cmap[agent_ind]
            label = "{}-SDC".format(ego_agent_id) if agent_ind == ego_agent_id else \
                "{}-OOI".format(agent_ind) if agent_ind in labeled_agent_id else str(agent_ind)
            ...
            x, y = agent_pos[agent_ind]

            raise ValueError("wrong animation")
            rect.set_xy((x - agent_shape[agent_ind, 0] / 2, y - agent_shape[agent_ind, 1] / 2))
            rect.angle = np.degrees(agent_heading[agent_ind])
...
def create_animation_from_trafficgen(data_dict, save_path='pred_animation.mp4', fps=10, dpi=300, draw_traffic=True):
    all_agent_shape = data_dict["decoder/current_agent_shape"]

    # The first agent is <s> in TrafficGen so we delete it.
    num_tg_agents = data_dict["decoder/modeled_agent_position_for_trafficgen"].shape[0] - 1

    _animate_trafficgen(
        save_path=save_path,
        agent_pos=data_dict["decoder/modeled_agent_position_for_trafficgen"][1:],
        agent_mask=np.ones(num_tg_agents, dtype=bool),
        agent_heading=data_dict["decoder/modeled_agent_heading_for_trafficgen"][1:],
        labeled_agent_id=[],  # TODO: ????
        ego_agent_id=0,  # TODO: ????
        agent_shape=all_agent_shape,
        data_dict=data_dict,
        dpi=dpi,
...
def debug(config):
    ...
    test_dataset = SceneStreamerDataset(config, "test")
